// 標準処理のjsspをロード
load('im_workflow/common/proc/exec/temporary_save_jssp');

function temporarySaveAsync(request){
  let asyncProcessWorkflow = new AsyncProcessWorkflow();
  let codeUtil = new WorkflowCodeUtil();
  let params = request.tempSaveParams;
  let resultInfo = {
    resultFlag : false,
    errorMessage : '',
    data : {
  ...
  let asyncProcessStatusData = {
    ...
    procType : codeUtil.getEnumCodeAsyncProcessType('asyncProcTyp_tempSave'),
  ...
  if (!result.resultFlag) {
    // 一時保存に失敗しました。
    resultInfo.errorMessage = MessageManager.getMessage('IMW.CLI.WRN.3622');
    return resultInfo;
  }

  let jsPathString = 'im_workflow/common/proc/exec/async/temporary_save_async_jssp';
  let parameter = {
...
function run(){
  ...
  logger.info('Start JS[temporary_save_async_jssp.js] run.');
  ...
  let resultInfo = temporarySave(requestObject);
  ...
  logger.info('Finish JS[temporary_save_async_jssp.js] run.');
}
