// 標準処理のjsspをロード
load('im_workflow/common/proc/exec/confirm_jssp');

function confirmAsync(request){
  let asyncProcessWorkflow = new AsyncProcessWorkflow();
  let codeUtil = new WorkflowCodeUtil();
  let params = request.confirmParams;
  let resultInfo = {
    resultFlag : false,
    errorMessage : '',
    data : {
  ...
  let asyncProcessStatusData = {
    ...
    executeUserCode : params.authUserCode,
    procType : codeUtil.getEnumCodeAsyncProcessType('asyncProcTyp_cnfm'),
    ...
    procComment : params.confirmComment,
    queueId : '',
  ...
  let jsPathString = 'im_workflow/common/proc/exec/async/confirm_async_jssp';
  let parameter = {
...
function run(){
  ...
  logger.info('Start JS[confirm_async_jssp.js] run.');
  ...
  // Contextをロード
  asyncProcessWorkflow.loadHTTPContext(_storedHttpContextId);

  // 確認処理実行
  let requestObject = ImJson.parseJSON(_param);
  let resultInfo = confirm(requestObject);
  if (!resultInfo.resultFlag) {
    ...
    let params = requestObject.confirmParams;
    asyncProcessStatusData.systemMatterId = params.systemMatterId;
    asyncProcessStatusData.nodeId = params.nodeId;
  ...
  logger.info('Finish JS[confirm_async_jssp.js] run.');
}
