...
function apply(parameter,userParameter) {
	...
	// ライセンシー用フローのみ、起案設定あり
	if(parameter.flowId == Constant.LO_FLOW_ACCOUNT){
		 kianSettingFlg = true;
	}
	...
	if (kianSettingFlg) {
		param.shinsei_status = Constant.LO_STATUS_TEISHUTSU;
...
function reapply(parameter,userParameter) {
	Constant.load("lo/common_libs/lo_const");
	Logger.getLogger().info("----- applyAction_account.reapply -----");
	var result = {
	              "resultFlag" : true,
	              "message"    : "",
	              "data"       : null
