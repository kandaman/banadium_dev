...
function statusUpdate(param) {
	...
	var result = db.select("SELECT * FROM lo_t_kyodaku WHERE kyodaku_id = ? and sakujo_flg ='0'", [DbParameter.string(param.kyodaku_id)]);
	if (result.data[0].shinsei_bi == null) {
		dataSet.shinsei_bi = DateTimeFormatter.format('yyyy/MM/dd', new Date());
	}
	// update条件の値を配列で持つ
