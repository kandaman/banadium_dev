...
function init(request) {
	...
	if ('shinsei_id' in request){
		...
		if ($userInfo.licenseeFlg == '1') {
			if($shinsei_data.kaisha_id !== $userInfo.userCompanyDepartment.companyCd){
				Content.executeFunction("lo/common_libs/lo_common_fnction", "toNoAuthorityPage");
			}
		...
		$flow_id = $shinsei_data.flow_id;
		
		//ノード一覧取得
		$nodeInfolist = Content.executeFunction("lo/contents/screen/account/account_data_retriever", "getNodeInfo",$flow_id);  
		
		//捺印情報取得
		if($shinsei_data.natsuin_change_flg =="1"){
			var natsuinResult = Content.executeFunction("lo/contents/screen/account/account_data_retriever", "retrieveShinseiKaisha", request.shinsei_id);
			$shinsei_data.natsuin_data = natsuinResult.data[0];
			
			//TODO
			$shinsei_data.natsuin_change_flg_nm ="変更あり";
		}else{
			var natsuinResult = Content.executeFunction("lo/contents/screen/account/account_data_retriever", "retrieveAccountKaisha", request.shinsei_id);
			$shinsei_data.natsuin_data = natsuinResult.data[0];
			
			//TODO
			$shinsei_data.natsuin_change_flg_nm ="変更なし";
		}
		...
		Logger.getLogger().info(' [init]　setMatterInfo');
		// 案件情報の取得		
		if (!setMatterInfo($shinsei_data)){
			// ない場合は新規でセット
			setWorkflowOpenPage($shinsei_data);	
			
			//ライセンシー用フローの場合は、起案ノードにタイトルに紐づくBNEグループをセットする
			if($flow_id == Constant.LO_FLOW_ACCOUNT){
    ...
    */
	...
	//最終確認ノードが空欄の場合は、申請ノードの情報を表示する（表示のみ）
	if(!$nodeUserslist.last_confirm || $nodeUserslist.last_confirm.userCd == "---"){
		$nodeUserslist.last_confirm = $nodeUserslist.apply;
	}
	*/
	
	//Logger.getLogger().info(' [init]　$nodeUserslist.' + ImJson.toJSONString(imwNodeSetting,true));
